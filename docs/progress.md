...
- [x] Configure Tailwind CSS with pastel/mint-celadon as primary theme color
...
- [x] Pastel/mint-celadon primary theme applied; light and dark modes working
...
- [x] **Blurred background shapes** – Full-page depth with heavily blurred pastel mint-celadon-colored orbs (`BlurredBackground.tsx`); six blobs at original-size scale (e.g. 45–80vmax, 260–420px, blur 78–100px); CSS variables `--blob-1`–`--blob-6` for light/dark; fixed layer behind content (z-0), main content `relative z-10`
- [x] **Unified scrolling background** – Section backgrounds and top borders removed (Timeline, Skills, Projects, Contact); single page background so content scrolls over one continuous pastel mint-celadon + blob layer
...
- [x] **Timeline vertical layout** – Vertical line down center; items alternate left/right (starting left); each item in a card showing title, company/institution, dates; click to expand/collapse details (responsibilities for work); “Click or tap cards to expand” hint; type icons on the line- [ ] Additional section or layout animations (to be added as we implement)
